...
**Step 3:** 
>Let $M^*$ be the result of deleting the row $t$ of $M$ $$Q^*=M^*(M^*)^T$$

Now in $M$ each column has only 2 entries. So after deleting one row even if $M^*$ didn't become an [[Incident]] matrix, the $(i,j)$th entry of  $M^*(M^*)^T$ still is the negative of number of edges in the graph generated by $Q^*$ i.e. $G-v_t$ since after deleting a row in $M$ the the other end of the edges passes through $v_t$ will be multiplied by 0 always in $M^*(M^*)^T$. Hence $Q^*=M^*(M^*)^T$. $\blacksquare$

If $m<n-1$ then the determinant is 0 and there are no [spanning subtrees](./Spanning%20Tree.md), so we assume that $m\geq n-1$. The [[Binet-Cauchy Formula]] computes the determinant of a product of non-square matrices using the determinants of square submatrices of the factors. When we apply the formula to $Q^*=M^*(M^*)^T$ the submatrix $A_S$ is an $(n-1)\times (n-1)$ submatrix of $M$ as discussed in Step 2 and $B_S= A_S^T$. Hence the summation counts $1=(\pm 1)^2$ for each set of $n-1$ edges corresponding to a [[Spanning Tree]] and 0 for all other sets of $n-1$ edges. $\blacksquare$
